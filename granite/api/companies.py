...
def list_companies(
    ...
    if include_deleted == 0:
        q = q.filter(CompanyRow.deleted_at.is_(None))

    # --- Спам-фильтр ---
    if include_spam == 0:
        # По умолчанию: скрыть spam
        q = q.filter(
            (EnrichedCompanyRow.segment != 'spam')
            | (EnrichedCompanyRow.segment.is_(None))
        )
    elif include_spam == 2:
        # Только спам
...
@router.post("/companies/{company_id}/mark-spam", response_model=OkResponse)
def mark_spam(
    company_id: int,
    body: MarkSpamRequest,
    db: Session = Depends(get_db),
):
    """Пометить компанию как спам.

    Операция:
    1. segment='spam' в companies + enriched_companies
    2. deleted_at = now() (скрывает из обычного списка)
    3. stop_automation = 1
    4. review_reason = 'mark-spam:{reason}:prev_segment={prev}'
    5. Audit touch
    """
    company = db.get(CompanyRow, company_id)
    if not company:
    ...
    if company.deleted_at is not None:
        raise HTTPException(400, "Company is already deleted")
    ...
    touch = CrmTouchRow(
        ...
        channel="manual",
        direction="outgoing",
        note=f"mark-spam: reason={body.reason}, prev_segment={prev_segment}",
    )
    db.add(touch)

    db.commit()
    logger.info(f"mark-spam: company {company_id} ({company.name_best}) -> spam, reason={body.reason}")
    return {"ok": True}


@router.post("/companies/{company_id}/unmark-spam", response_model=OkResponse)
def unmark_spam(
    company_id: int,
    db: Session = Depends(get_db),
):
    ...
    Операция:
    1. Восстановить segment из review_reason (prev_segment=X)
    2. deleted_at = None
    3. stop_automation = 0
    4. Audit touch
    """
    ...
    # Извлекаем предыдущий сегмент из review_reason
    prev_segment = "D"  # fallback
    if company.review_reason and "prev_segment=" in company.review_reason:
        for part in company.review_reason.split(":"):
            if part.startswith("prev_segment="):
                prev_segment = part.split("=")[1]
                break

    # 1. Восстанавливаем
    company.segment = prev_segment
    company.deleted_at = None
    company.review_reason = ""
    company.needs_review = False
    company.updated_at = datetime.now(timezone.utc)

    # Если status был spam — сбрасываем
    if company.status == "spam":
        company.status = "active"

    # 2. Синхронизируем enriched
    enriched = db.get(EnrichedCompanyRow, company_id)
    if enriched:
        enriched.segment = prev_segment

    # 3. Сбрасываем stop_automation
    contact = db.get(CrmContactRow, company_id)
    if contact:
        contact.stop_automation = 0
        contact.updated_at = datetime.now(timezone.utc)
    ...
    touch = CrmTouchRow(
        ...
        channel="manual",
        direction="outgoing",
        note=f"unmark-spam: restored segment={prev_segment}",
    )
    db.add(touch)

    db.commit()
    logger.info(f"unmark-spam: company {company_id} ({company.name_best}) -> segment={prev_segment}")
    return {"ok": True}


@router.post("/companies/{company_id}/mark-duplicate", response_model=OkResponse)
def mark_duplicate(
    company_id: int,
    body: MarkDuplicateRequest,
    db: Session = Depends(get_db),
):
    """Пометить компанию как дубликат (лёгкий merge).

    В отличие от полного merge (PATCH /companies/{id}/merge):
    - НЕ переносит данные (телефоны, emails)
    - Только: merged_into = target_id + deleted_at + audit
    """
    source = db.get(CompanyRow, company_id)
    if not source:
        raise HTTPException(404, "Source company not found")
    if source.deleted_at is not None:
        raise HTTPException(400, "Source company is already deleted")
    if company_id == body.target_id:
        raise HTTPException(400, "Cannot mark company as duplicate of itself")
    ...
    source.deleted_at = datetime.now(timezone.utc)
    source.review_reason = f"duplicate_of:{body.target_id}"
    ...
    touch = CrmTouchRow(
        ...
        channel="manual",
        direction="outgoing",
        note=f"mark-duplicate: source={company_id}, target={body.target_id}",
    )
    db.add(touch)

    db.commit()
    logger.info(f"mark-duplicate: {company_id} ({source.name_best}) -> duplicate of {body.target_id}")
    return {"ok": True}


@router.post("/companies/{company_id}/resolve-review", response_model=OkResponse)
def resolve_review(
    company_id: int,
    body: ResolveReviewRequest,
    db: Session = Depends(get_db),
):
    """Разрешить needs_review для компании.

    Actions:
    - approve: очистить needs_review + review_reason
    - spam: делегировать mark-spam (требуется reason)
    - duplicate: делегировать mark-duplicate (требуется target_id)
    """
    company = db.get(CompanyRow, company_id)
    if not company:
        raise HTTPException(404, "Company not found")
    ...
    if body.action == "approve":
        # Просто подтверждаем — очищаем флаг
        company.needs_review = False
        company.review_reason = ""
        company.updated_at = datetime.now(timezone.utc)
        ...
        touch = CrmTouchRow(
            ...
            channel="manual",
            direction="outgoing",
            note="resolve-review: approved",
        )
        db.add(touch)
        db.commit()
        logger.info(f"resolve-review: company {company_id} approved")
        return {"ok": True}

    elif body.action == "spam":
        if not body.reason:
            raise HTTPException(422, "reason is required for action=spam")
        # Делегируем mark-spam
        prev_segment = company.segment or "unknown"
        company.segment = "spam"
        company.status = "spam"
        company.deleted_at = datetime.now(timezone.utc)
        ...
        touch = CrmTouchRow(
            ...
            channel="manual",
            direction="outgoing",
            note=f"resolve-review: spam, reason={body.reason}",
        )
        db.add(touch)
        db.commit()
        logger.info(f"resolve-review: company {company_id} -> spam")
        return {"ok": True}
        ...
        company.merged_into = body.target_id
        company.deleted_at = datetime.now(timezone.utc)
        company.review_reason = f"duplicate_of:{body.target_id}"
        company.needs_review = False
        company.updated_at = datetime.now(timezone.utc)
        ...
        touch = CrmTouchRow(
            ...
            channel="manual",
            direction="outgoing",
            note=f"resolve-review: duplicate, target={body.target_id}",
        )
        db.add(touch)
        db.commit()
        logger.info(f"resolve-review: company {company_id} -> duplicate of {body.target_id}")
        return {"ok": True}
