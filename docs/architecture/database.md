...
### 2.7 crm_templates — шаблоны сообщений ⚠️ DEPRECATED
...
| `retired` | BOOLEAN, NOT NULL | Архивный/immutable шаблон (default False) — **не используется** |
| `created_at` | DATETIME | Время создания |
| `updated_at` | DATETIME | Время обновления |

**Новый способ работы с шаблонами:** `EmailTemplate` из `TemplateRegistry` (`granite/templates.py`). Методы: `render(**kwargs)`, `render_subject(**kwargs)`. Загрузка из `data/email_templates.json` при старте приложения. Перезагрузка без рестарта: `POST /api/v1/templates/reload` или `uv run cli.py templates-reload`.
...
| `template_id` | INTEGER FK | ID immutable-шаблона → `crm_templates.id` (nullable, **deprecated**) |
| `rendered_body` | TEXT, nullable | Plain text отправленного письма (для истории, ~1-2 КБ) |
| `created_at` | DATETIME | Время создания |

Индексы: `ix_crm_email_logs_company_id`, `ix_crm_email_logs_campaign_id`, `ix_crm_email_logs_tracking_id`.

> **rendered_body** — новое поле, добавлено миграцией `20260429_120000_add_rendered_body_to_email_logs`. Хранит plain text рендер письма (~1-2 КБ), чтобы просмотреть отправленное письмо без повторного рендеринга шаблона. Для HTML-шаблонов хранится plain text, извлечённый через `_html_to_plain_text()`.
...
└─────────────────┘                                     │ error_message   │
          ...
          ▼
┌─────────────────┐         ┌─────────────────────┐
│ crm_tasks       │         │ crm_templates ⚠️DEP │
├─────────────────┤         ├─────────────────────┤
│ id (PK)         │         │ id (PK)             │
│ company_id (FK) │         │ name (UNIQUE)       │
│ title           │         │ channel, subject    │
│ description     │         │ body, body_type     │
│ due_date        │         │ retired (не исп.)   │
│ priority,status │         └─────────────────────┘
│ task_type       │
│ completed_at    │         ┌──────────────────────┐
└─────────────────┘         │crm_email_campaigns   │
...
| `20260429_120000` | 2026-04-29 | `add_rendered_body_to_email_logs` — колонка `rendered_body` (Text, nullable) |
...
└─────────────────┘     └─────────┬─────────┘
          ...
          │ (tracking pixel, open rate,  │  через TemplateRegistry
          │  rendered_body)              │  (email_templates.json)
          └──────────────────────────────┘
                 │
                 ▼
          ┌──────────────────────────────┐
