# RetouchGrav — Email Campaign Dev Plan v10
...
> v10 · 2026-04-26 · v9 + убрать телефон из шаблонов, фикс process_replies (авто-отмена follow-up), открытые вопросы

---

## Содержание

1. [Что изменилось относительно v9](#1-что-изменилось-относительно-v9)
...
## 1. Что изменилось относительно v9
...
| `process_replies.py` напрямую меняет `funnel_stage`, не вызывая логику отмены follow-up | ✅ Добавлен вызов `_cancel_followup_tasks()` после перевода в `replied` | Без этого авто-отмена follow-up задач из задачи 5.2 не срабатывает при обнаружении ответа |
...
Вынести в отдельную функцию `_cancel_followup_tasks()` — она нужна и в `stage_transitions.py`, и в `process_replies.py` (см. задачу 9):

```python
# granite/api/stage_transitions.py (или отдельный helper)

CANCEL_FOLLOWUP_ON_STAGES = {"replied", "interested", "not_interested", "unreachable"}


def _cancel_followup_tasks(company_id: int, new_stage: str, db: Session) -> None:
...
```python
_cancel_followup_tasks(company_id, new_stage, db)
```
...
**Итоговый regex после фикса (v10):**
...
**v10: добавлен вызов `_cancel_followup_tasks()`** — исправляет проблему, обнаруженную при ревью v9.
...
def process_replies() -> int:
    ...
    from granite.api.stage_transitions import _cancel_followup_tasks  # v10
    from sqlalchemy.orm import Session
    ...
    with imaplib.IMAP4_SSL(IMAP_HOST) as imap:
        ...
        for uid in uids[0].split():
            ...
            with Session(engine) as session:
                ...
                # v10: отменить pending follow-up задачи
                _cancel_followup_tasks(contact.company_id, "replied", session)
...
*v10 · 2026-04-26 · v9 + убран телефон из шаблонов, фикс process_replies (вызов _cancel_followup_tasks), заметка о поведении BackgroundTask при рестарте, открытые вопросы вынесены в раздел 7*
