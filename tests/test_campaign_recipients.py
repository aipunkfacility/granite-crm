...
class TestNetworkEmailFallback:
    ...
    def test_enriched_row_fallback_when_company_email_row_empty(self, db_session, client):
        """Fallback uses EnrichedCompanyRow.emails when CompanyEmailRow is empty.

        Real scenario: pipeline scraped emails, stored them in EnrichedCompanyRow.emails,
        but sync_company_emails was never called -- CompanyEmailRow is empty.
        The fallback should still find peer emails via EnrichedCompanyRow.
        """
        c1 = CompanyRow(name_best="C1", city="москва",
                        website="https://guravli.agency/msk",
                        emails=["info@guravli.agency"])
        ...
        # Deliberately NOT calling sync_company_emails -- CompanyEmailRow stays empty
        # Only EnrichedCompanyRow has emails
        db_session.add(EnrichedCompanyRow(id=c1.id, name=c1.name_best,
                      city=c1.city, website=c1.website, emails=c1.emails,
                      is_network=True))
        db_session.add(EnrichedCompanyRow(id=c2.id, name=c2.name_best,
        ...
        assert resp.status_code == 200
        data = resp.json()
        # Both companies have no CompanyEmailRow entry, fallback finds
        # info@guravli.agency from c1's EnrichedCompanyRow and assigns to c2.
        # c1 also gets its own email from EnrichedRow. Dedup -> 1 added.
        assert data["added"] == 1, (
            f"Expected 1 added (enriched row fallback), "
            f"got {data['added']}: {data.get('skipped_details')}"
        )
