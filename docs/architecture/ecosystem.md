...
RetouchGrav — не монолит, а экосистема из четырёх независимо развёртываемых компонентов и одного общего ресурса:

| Компонент | Репозиторий | Среда/Хостинг | Стек |
|-----------|------------|---------|------|
| AI-ретушь | granite-retouch (MEMORIAL) | Google Antigravity IDE + GIMP CLI | Мультиагентный пайплайн, Python |
...
Каждый компонент можно обновлять и деплоить независимо. Связь — через URL, REST API и общие файлы (email-шаблон). Прямой интеграции granite-retouch ↔ granite-crm пока нет — ретушь и продажи работают как параллельные потоки.

---

## 2. AI-ретушь (granite-retouch)

**Репозиторий:** github.com/aipunkfacility/MEMORIAL
**Среда:** Google Antigravity IDE (мультиагентный пайплайн)
**Назначение:** Автоматизированная подготовка портретов для гравировки на камне — сырая фотография на входе, готовый TIFF/BMP на выходе.

### Агентный пайплайн

```
source.jpg → Analyzer → Prompter → Nano Banana → GIMP CLI → final.tiff
```

| Агент | Что делает | Выход |
|-------|-----------|-------|
| **memorial-analyzer** | Анализирует фото: тип одежды, ткань, головной убор, качество лица, дефекты | `analyzer_output` в order.json |
| **memorial-prompter** | Собирает промпт из атомарных блоков (base → clothing → headgear → machine) | `final_prompt` для Nano Banana |
| **Генерация** | Оператор запускает генерацию в Nano Banana на синем хромакее `#0000FF` | ai.png |
| **GIMP-постпроцессинг** | Агенты через CLI Anything: Ч/Б, удаление хромакея, Inner Glow, Levels, Unsharp, виньетирование, экспорт TIFF | final.tiff |

### Типы станков и требования

| Станок | Оборудование | Кожа | Контур (Inner Glow) |
|--------|-------------|------|---------------------|
| **Лазер** (Mirtels, Stanzone) | Плавит камень, мягкие полутона | Гладкая, пористость минимальна | 40-80px, 30-40% opacity |
| **Ударный/Игла** (Sauno, Зубр) | Алмазная игла, растр из точек | High Key, микро-контраст пор обязателен | 10-25px, 60-80% opacity |

### Фундаментальные принципы

1. **Синий хромакей (#0000FF)** — при гашении синего канала даёт идеально чёрный контур без ореолов
2. **Разделение текстур** — гладкое лицо для объёма + сверхрезкая одежда для станка
3. **Идентичность** — строгий запрет на Face Swap, сохранение геометрии лица
4. **Контурный свет (Rim Light)** — камень «съедает» границы, без искусственного контура голова «летает»

### Связь с остальной экосистемой

granite-retouch производит портреты — это продукт, который RetouchGrav продаёт. CRM находит клиентов и продаёт, а granite-retouch обеспечивает производство. Прямой передачи файлов между ними пока нет — оператор вручную передаёт готовые файлы мастерским.

---

## 3. Лендинг (monument-web)
...
### Сценарий 0: Производство ретуши (granite-retouch)

AI-ретушь — отдельный поток, который работает параллельно с продажами:

```
1. Клиент присылает фото (через TG/WA/email)
   → Оператор создаёт заказ в granite-retouch

2. Analyzer анализирует фото (одежда, дефекты, качество)
   → Prompter собирает промпт из атомарных блоков

3. Оператор генерирует в Nano Banana на синем хромакее #0000FF
   → GIMP CLI: Ч/Б, удаление фона, Inner Glow, Levels, Unsharp
   → final.tiff готов к гравировке

4. Оператор отправляет результат клиенту вручную
   (автоматической интеграции с CRM пока нет)
```

### Сценарий 1: Скрапинг → Кампания → Лендинг

Основной сценарий продаж — найти мастерские и установить контакт:
...
## 8. Хостинг изображений (memorial-img.netlify.app)
