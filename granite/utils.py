...
_SEO_TITLE_PATTERN = re.compile(
    ...
    r"памятник[аиы]?\s*(?:из|в|на|от|и)\s+\S|"  # предлог обязателен + слово после (v13: возврат к v11)
    r"изготовлен.*(?:памятник|надгробие)|"       # без «гранит» (v7: «Гранит» — норма для ниши)
    r"памятники\s*(?:в|из|на|и)\s+\S|"          # "Памятники в/из" + слово (scraper-audit A-7)
    r"памятники\s*(?:на\s*кладбищ)|"             # "Памятники на кладбищ"
    r"изготовление\s*памятников|"                # "Изготовление памятников" (scraper-audit A-7)
    r"памятники\s*и\s*надгробия|"                # "Памятники и надгробия" (scraper-audit A-7)
    r"производство\s*памятников|"
    # B2: Паттерны для слипшихся слов (SEO-краулеры сливают слова)
