...
**Важно:** При пакетном запуске (через `all` или `--city-list`) по завершении всех городов автоматически запускается команда `scan-networks` (A-8) для выявления крупных сетей агрегаторов.

Между городами выдерживается пауза (`inter_city_delay` в config.yaml) для предотвращения блокировок поисковиков.
...
### `scan-networks` — глобальный сетевой сканер (A-6/A-8)

```bash
uv run cli.py scan-networks
```

Ищет домены сайтов, которые встречаются в 3 и более городах проекта. Помечает такие записи как `needs_review=True` с причиной `network_aggregator_detected`. Команда запускается автоматически после завершения пайплайна для списка городов.
