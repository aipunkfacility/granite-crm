# RetouchGrav — Email Campaign Dev Plan v7
...
> v7 · 2026-04-26 · Правки ревью v6 + реальность кода

---

## Содержание

1. [Что изменилось относительно v6](#1-что-изменилось-относительно-v6)
...
## 1. Что изменилось относительно v6

| В v6 | В v7 | Почему |
|------|------|--------|
| SPF/DKIM/DMARC настраиваются на своём домене | ❌ Убрано — отправка через Gmail | Google управляет SPF/DKIM/DMARC для gmail.com. Вы не можете добавить свои записи. Письма уже подписываются Google DKIM автоматически |
| DKIM-подпись в sender.py | ❌ Убрано | Невозможно через Gmail SMTP — Google подписывает своим ключом. Код `dkim.sign()` в v6 содержал баг с парсингом `sig` и всё равно не работал бы через gmail.com |
| Заголовки List-Unsubscribe / List-Unsubscribe-Post | ❌ Убрано | Gmail фильтрует произвольные заголовки при отправке через SMTP. One-Click Unsubscribe через gmail.com не работает. Отписка — только через ссылку в теле письма |
| `SELECT FOR UPDATE` для блокировки | ❌ Исправлено | SQLite не поддерживает `SELECT FOR UPDATE`. В коде уже есть Python `threading.Lock()` + атомарный `UPDATE ... WHERE status NOT IN (...)` — это работает |
| Recovery через `@app.on_event("startup")` | ❌ Исправлено | `on_event` депрекейтнут. В app.py уже есть `lifespan()` — recovery добавляется туда |
| A/B тест: «победитель ≥ 3% + превышение на 50%» | ⚠️ Упрощено | На 25/25 писем 1 ответ = 4% vs 0% — это шум, не статистика. Новый критерий ниже |
| Автоматический resume при `paused_daily_limit` | ❌ Не нужен | Ручной перезапуск — нормально |
| `{company_name}` убрано, ничего взамен | ✅ Добавлен `{city}` | «Ищу контакты мастерских в {city}» — персонализация без риска SEO-мусора |
| Bounce-парсинг: паттерн `"550"` в тексте | ❌ Исправлено | Ложные срабатывания (IP-адреса, размеры). Используем regex по SMTP-кодам |
| Валидатор не проверяет формат email | ✅ Добавлено | Минимальная проверка: `info@`, `@domain.ru` — не валидные email |
| GET /unsubscribe сразу отписывает | ⚠️ Добавлена защита | Почтовые клиенты префетчат ссылки. Для GET — показываем страницу подтверждения |
...
> **Важно по волне 1 / Marquiz:** Перед запуском вручную проверить список — убедиться что `company_name` не SEO-мусор. 22 компании — проверяется за 10 минут.
...
[ ] .env: IMAP_HOST=imap.gmail.com (для bounce)
...
### День 1 — Инфраструктура (0 писем)

```
[ ] Gmail: 2FA + App Password
[ ] .env: SMTP + IMAP + BASE_URL
[ ] Тест: отправить письмо себе через sender.py
...
*v7 · 2026-04-26 · Gmail SMTP, unsubscribe POST, BackgroundTask + SSE, упрощённый A/B, исправленный bounce-парсинг*
