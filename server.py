...
async def process_batch(job_id: str, contacts: list[BatchContact], html: str):
    """Process batch email sending in background, updating job after each email."""
    job = jobs[job_id]
    ...
    for i, contact in enumerate(contacts):
        ...
        try:
            ...
            server.sendmail(
                config["sender_email"], contact.email, msg.as_string()
            )
        ...
        # Yield to event loop so status polls get served
