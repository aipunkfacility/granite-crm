...
> v13 · 2026-04-28 · v12 + 4 баг-фикса + возврат кода из v11 + новая задача 19 + структурные улучшения + Этапы 3–4 завершены
...
| Commit-per-task в process_followups | Каждый follow-up таск коммитится отдельно — как в campaigns/bounces/replies | Этап 3 |
| Retry limit для follow-up | 3 неудачные отправки → status='failed', retry_count в description | Этап 3 |
| Bounce email: cascade fallback | X-Failed-Recipients → Return-Path → From — mailer-daemon в From бесполезен | Этап 3 |
| Спам-жалоба отменяет follow-up | cancel_followup_tasks() вызывается немедленно при спам-жалобе | Этап 3 |
| Тесты Этапа 3 реорганизованы по фичам | test_phase3.py → test_imap_helpers/test_bounces/test_replies/test_followup | Этап 3 |
| Watchdog: max() вместо priority для timestamps | max(updated_at, started_at, created_at) — честно берёт самый свежий timestamp | Этап 4 |
| updated_at default при создании кампании | CrmEmailCampaignRow.updated_at получает default — не NULL при создании | Этап 4 |
| CampaignFilters — Pydantic модель вместо dict | Типобезопасные фильтры в UpdateCampaignRequest, валидация на уровне схемы | Этап 4 |
| Тесты Этапа 4 реорганизованы по фичам | test_phase4.py → test_campaign_wizard/test_reply_api | Этап 4 |
| PostReply: только нажатая кнопка блокируется | Не все кнопки disable при отправке одной — UX корректный | Этап 4 |
| WA не перезаписывает tg_sent в stage_transitions | WhatsApp touch не затирает уже установленный tg_sent stage | Этап 4 |
...
### Этап 3: Обратная связь — follow-up, bounce, reply, IMAP helpers (10-20 реальных писем) ✅ ВЫПОЛНЕН
...
| Задача | Что делаем | Статус |
|--------|-----------|--------|
| **19. IMAP helpers module** (НОВОЕ v13) | `granite/email/imap_helpers.py`: `extract_email()`, `extract_body()`, `is_bounce()`, `is_ooo()`, `extract_bounced_email()`, `extract_dsn()`, `fetch_imap_messages()` | ✅ Все функции + 15 тестов |
| **6. Bounce parser** | `process_bounces.py`: IMAP → bounce → `CrmEmailLogRow.status="bounced"`, `CrmContactRow.funnel_stage="unreachable"`. Cascade fallback: X-Failed-Recipients → Return-Path → From | ✅ DSN 5.1.1/5.2.2/5.7.1 + IMAP error + X-Failed-Recipients fallback + 6 тестов |
| **9. Reply parser** | `process_replies.py`: IMAP → ответ → `funnel_stage="replied"`, `cancel_followup_tasks()`, `total_replied++`. Спам-жалоба → `stop_automation=1` + `cancel_followup_tasks()` | ✅ Reply/OOO/spam + cancel on spam + Auto-Submitted RFC 3834 + 8 тестов |
| **5. Follow-up + счётчики** | `maybe_create_followup_task()`: задача +7д. `cancel_followup_tasks()`: отмена при терминальной стадии. `increment_campaign_opened()`: total_opened++. Title без None для non-campaign | ✅ Создание/отмена/счётчики + title fix + 4 теста |
| **11. Follow-up executor** | `process_followups.py`: созревшие задачи → отправить → completed. Commit-per-task (как в campaigns). Retry limit: 3 → `status="failed"`. Тема `Re: {original_subject}` | ✅ Executor + commit-per-task + retry limit + 7 тестов |
...
8. ~~`uv run pytest tests/ -v` — всё зелёное~~ ✅ 40 тестов этапа 3 в feature-based файлах

**Аудит Этапа 3 (пост-имплементация):**

После реализации основного кода (`dfb5475`) проведён аудит, найдено 12 проблем:

| ID | Серьёзность | Проблема | Фикс |
|----|------------|----------|------|
| P3-H1 | HIGH | `process_followups()` — один commit в конце цикла (потеря данных при сбое) | ✅ Commit-per-task внутри цикла |
| P3-H2 | HIGH | Нет лимита повторных попыток для follow-up (бесконечный retry) | ✅ retry_count в description, лимит 3 → `status="failed"` |
| P3-H3 | HIGH | Bounce email: только From — mailer-daemon@ бесполезен | ✅ Cascade: X-Failed-Recipients → Return-Path → From |
| P3-M1 | MED | Мёртвая переменная `content_type` в `is_bounce()` | ✅ Убрана |
| P3-M2 | MED | Мёртвая переменная `fetch_flag` в `fetch_imap_messages()` | ✅ Убрана |
| P3-M3 | MED | Спам-жалоба не отменяет follow-up немедленно | ✅ `cancel_followup_tasks()` при спам-жалобе |
| P3-M4 | MED | Title с `None` для non-campaign писем | ✅ Условное форматирование без None |
| P3-L1 | LOW | Нет теста для `Auto-Submitted: auto-replied` (RFC 3834) | — (покрыто в P3-L2) |
| P3-L2 | LOW | Нет теста для `Auto-Submitted: auto-replied` | ✅ Тест добавлен |
| P3-L3 | LOW | `test_phase3.py` — монолитный файл по «фазе», а не по модулю | ✅ Реорганизован (см. ниже) |
| P3-L4 | LOW | Нет логирования в `process_followups.py` | — Отложено (не блокирует) |

**Реорганизация тестов:** `0739b93`

`test_phase3.py` (39 тестов) → разбиты по фичам + 1 новый тест:

| Файл | Классы | Тестов |
|------|--------|--------|
| `tests/test_imap_helpers.py` | TestExtractEmail (3), TestExtractBody (2), TestIsBounce (2), TestIsOoo (4), TestExtractBouncedEmail (2), TestExtractDsn (2) | 15 |
| `tests/test_bounces.py` | TestBounceParser (6) | 6 |
| `tests/test_replies.py` | TestReplyParser (8) | 8 |
| `tests/test_followup.py` | TestFollowupCreation (3), TestFollowupCancellation (2), TestTotalOpened (1), TestFollowupExecutor (5) | 11 |

Итого: **40 тестов** этапа 3 (15+6+8+11)

**Коммиты:** `dfb5475` (Этап 3), `bc54c30` (аудит P3-H1..H3, P3-M1..M4, P3-L2), `0739b93` (реорганизация тестов)

**Дополнительные файлы, созданные в Этапе 3:**

| Файл | Назначение |
|------|------------|
| `granite/email/imap_helpers.py` | IMAP-хелперы: extract_email, extract_body, is_bounce, is_ooo, extract_bounced_email, extract_dsn, fetch_imap_messages |
| `granite/email/process_bounces.py` | Обработка bounce: DSN-коды, cascade fallback email, graceful IMAP error |
| `granite/email/process_replies.py` | Обработка ответов: reply/OOO/spam, cancel_followup на спам, Auto-Submitted RFC 3834 |
| `granite/email/process_followups.py` | Исполнитель follow-up: commit-per-task, retry limit (3→failed), Re: тема |
| `granite/email/followup_logic.py` | Логика follow-up: maybe_create_followup_task, cancel_followup_tasks, increment_campaign_opened |
| `granite/api/tracking.py` | Обновлён: вызывает maybe_create_followup_task + increment_campaign_opened при tracking pixel |

**Зависимости:** этапы 1+2 завершены ✅

---

### Этап 4: UI + финализация (запуск Волны 1) ✅ ВЫПОЛНЕН

**Цель:** фронтенд для управления кампаниями, post-reply UI, dashboard с A/B-статистикой. После этого этапа — полноценный запуск первой волны на 50 компаний.

**Принцип TDD:** для API-изменений — обычные юнит-тесты. Для UI — ручное тестирование + API-level тесты.

| Задача | Что делаем | Статус |
|--------|-----------|--------|
| **10. Фронтенд** | Wizard создания кампании (шаблон + A/B + фильтры). Карточка компании с post-reply кнопками. Dashboard со статистикой | ✅ CampaignWizard + CampaignDashboard + PostReplyButtons + TouchHistory |
| **10a. Reply API** | POST /companies/{id}/reply + POST /companies/{id}/reply/preview — отправка и превью ответа из карточки компании | ✅ Оба эндпоинта + 11 тестов |
| **10b. Campaign API расширения** | POST /campaigns/preview-recipients, CampaignFilters.cities[], validator warnings, subject_a/subject_b в CampaignResponse | ✅ preview-recipients + multi-city + warnings + A/B fields |
| **12 (доки)** | `docs/POST_REPLY_PLAYBOOK.md` + `docs/EMAIL_TEMPLATES.md` | ✅ Созданы |
| **13. Bitrix-шаблон** | Шаблон `cold_email_bitrix` (id=4) добавлен в `email_templates.json` | ✅ Шаблон #4 в JSON |

**Порядок реализации (этап 4):**
1. ~~Задача 10a: Reply API — POST /reply + POST /reply/preview~~ ✅
2. ~~Задача 10b: Campaign API расширения~~ ✅
3. ~~Задача 10: Фронтенд — wizard + карточка + dashboard~~ ✅
4. ~~Задача 12: Документация~~ ✅
5. ~~Задача 13: Bitrix-шаблон~~ ✅
6. ~~`uv run pytest tests/ -v` — всё зелёное~~ ✅ 28 тестов этапа 4 в feature-based файлах
7. Прогрев домена (3 дня по 5 писем на свои адреса) — ⚠️ не проводился
8. mail-tester.com → оценка ≥ 8/10 — ⚠️ не проводился
9. Запуск Волны 1: 50 компаний, A/B тест тем — ⚠️ не проводился

**Аудиты Этапа 4 (пост-имплементация):**

После реализации основного кода (`f150c14`) проведено 3 раунда аудита + 1 реорганизация тестов:

| Раунд | Коммит | Найдено | Исправлено |
|-------|--------|---------|----------|
| Аудит 1 | `95b5476` | P4-H1..H4, P4-M2..M6, P4-L2,L5 (11 проблем) — reply API guards, campaign status guards, A/B fields, UX fixes | ✅ Все 11 |
| Аудит 2 | `9bac878` | P4R-H1..P4R-L19 (52 исправления) — расширенный аудит: типизация, UX, edge cases, schemas | ✅ Все 52 |
| Реорганизация тестов | `415b9fc` | test_phase4.py → test_campaign_wizard.py + test_reply_api.py | ✅ Feature-based |
| Аудит 3 | `4293813` | 10 багов (1 HIGH, 5 MEDIUM, 4 LOW) — city filter, timezone, total_recipients, button UX, schemas, stage transitions, watchdog, typing | ✅ Все 10 |

**Аудит 1 — детали (P4-H1..H4, P4-M2..M6, P4-L2,L5):**

| ID | Серьёзность | Проблема | Фикс |
|----|------------|----------|------|
| P4-H1 | HIGH | Reply API: нет проверки stop_automation перед отправкой | ✅ Проверка + 409 |
| P4-H2 | HIGH | Campaign API: нет guard от PATCH/DELETE running/completed кампаний | ✅ 409 для running/completed |
| P4-H3 | HIGH | CampaignWizard: subject_a/subject_b не передаются на бэкенд | ✅ Поля добавлены |
| P4-H4 | HIGH | PostReplyButtons: нет обработки ошибок при отправке | ✅ Toast на ошибку |
| P4-M2 | MED | CampaignDashboard: не показывает validator_warnings | ✅ Отображение warnings |
| P4-M3 | MED | CampaignWizard: нет превью получателей перед созданием | ✅ preview-recipients endpoint |
| P4-M4 | MED | PostReplyButtons: все кнопки блокируются при отправке одной | ✅ Только нажатая блокируется |
| P4-M5 | MED | TouchHistory: нет отображения template_name | ✅ template_name в touch |
| P4-M6 | MED | campaigns.ts: нет типа для CampaignFilters | ✅ Тип добавлен |
| P4-L2 | LOW | CampaignDashboard: A/B данные нетипизированы (any) | ✅ Типизировано |
| P4-L5 | LOW | PostReplyButtons: нет лоадера при отправке | ✅ Spinner на кнопке |

**Аудит 2 — детали (P4R-H1..P4R-L19, 52 исправления):**

Расширенный аудит, включающий:
- Типизация: CampaignFilters тип вместо dict, A/B variant typed, stop_automation_warning тип
- UX: улучшенные сообщения об ошибках, лоадеры, валидация форм
- Edge cases: пустые фильтры, отсутствующие шаблоны, невалидные данные
- Schemas: CampaignFilters как Pydantic модель, валидация cities

**Аудит 3 — детали (10 багов, 1 HIGH, 5 MEDIUM, 4 LOW):**

| ID | Серьёзность | Проблема | Фикс |
|----|------------|----------|------|
| A1 | HIGH | CampaignWizard: `__all__` literal отправляется как город-фильтр | ✅ Маппинг на undefined (нет фильтра) |
| A2 | MED | validator.py: `.replace(tzinfo=)` крашит на уже timezone-aware timestamp | ✅ `.astimezone()` для aware timestamps |
| A3 | MED | page.tsx: `||` для total_recipients — 0 проваливается к total_sent | ✅ `??` вместо `||` |
| A4 | MED | PostReplyButtons: все кнопки disable при клике одной | ✅ Только нажатая disable + spinner |
| A5 | MED | schemas.py: filters как `Optional[dict]` вместо `Optional[CampaignFilters]` | ✅ Типизировано как CampaignFilters |
| A6 | MED | replies.ts + PostReplyButtons: `(result as any).stop_automation_warning` | ✅ Тип stop_automation_warning в SendReplyResult |
| A7 | LOW | stage_transitions.py: WA перезаписывает tg_sent | ✅ WA не перезаписывает tg_sent |
| A8 | LOW | database.py: updated_at = NULL при создании кампании | ✅ default=lambda: datetime.now(timezone.utc) |
| A9 | LOW | CampaignDashboard: A/B variant data как `any` | ✅ Типизировано явно |
| A10 | LOW | TouchHistory: нет пробела перед «показано» | ✅ Проверено — пробел уже есть |

**Регрессия от A8:** Добавление default на updated_at сломало watchdog-тесты (updated_at теперь всегда свежий). Фикс: watchdog использует `max(updated_at, started_at, created_at)` вместо приоритетной логики `updated_at or started_at or created_at`. Тесты обновлены: явно задаются старые timestamp.

**Реорганизация тестов:** `415b9fc`

`test_phase4.py` (339 строк, 17 тестов) → разбиты по фичам + 11 новых тестов:

| Файл | Классы | Тестов |
|------|--------|--------|
| `tests/test_campaign_wizard.py` | TestCampaignABSubjects (4), TestCampaignFilters (2), TestPreviewRecipients (2), TestValidatorWarnings (2), TestABStats (2), TestUpdateCampaignStatusGuard (2), TestDeleteCampaign (3) | 17 |
| `tests/test_reply_api.py` | TestPreviewReply (5), TestSendReply (6) | 11 |

Итого: **28 тестов** этапа 4 (17 + 11)

**Коммиты:** `f150c14` (Этап 4), `95b5476` (аудит 1), `9bac878` (аудит 2), `415b9fc` (реорганизация тестов), `4293813` (аудит 3)

**Дополнительные файлы, созданные/изменённые в Этапе 4:**

| Файл | Назначение |
|------|------------|
| `granite/api/replies.py` | Reply API: POST /reply + POST /reply/preview + send через EmailSender |
| `granite/api/campaigns.py` | Расширения: preview-recipients, multi-city filter, validator warnings, status guards, watchdog fix |
| `granite/api/schemas.py` | CampaignFilters (Pydantic), UpdateCampaignRequest.filters typed, CampaignResponse + A/B fields |
| `granite/api/stage_transitions.py` | WA не перезаписывает tg_sent |
| `granite/email/validator.py` | Безопасная конвертация timezone-aware timestamps |
| `granite/database.py` | updated_at default на создание кампании |
| `granite-web/src/components/campaigns/CampaignWizard.tsx` | 4-шаговый wizard: Template → Filters → A/B → Confirm (552 строки) |
| `granite-web/src/components/campaigns/CampaignDashboard.tsx` | Dashboard: SSE live-progress, A/B сравнение, pause reason, validator warnings (393 строки) |
| `granite-web/src/components/companies/PostReplyButtons.tsx` | Кнопки post-reply с template preview + stop_automation guard (202 строки) |
| `granite-web/src/components/companies/TouchHistory.tsx` | История касаний на карточке компании (133 строки) |
| `granite-web/src/app/campaigns/page.tsx` | Страница кампаний с wizard + dashboard (295 строк) |
| `granite-web/src/lib/api/campaigns.ts` | API клиент: CampaignFilters, preview-recipients, A/B fields (124 строки) |
| `granite-web/src/lib/api/replies.ts` | API клиент: preview + send reply, SendReplyResult тип (28 строк) |
| `granite-web/src/lib/hooks/use-campaigns.ts` | React Query hook для кампаний (29 строк) |
...
| 3 ✅ | **19**, 5, 6, 9, 11 | CRM обрабатывает bounce/reply/follow-up автоматически + IMAP helpers | ✅ Завершён (3 коммита + аудит) |
| 4 ✅ | 10, 10a, 10b, 12(доки), 13 | Полный цикл: UI → кампания → статистика → post-reply + 3 аудита | ✅ Завершён (5 коммитов + 3 аудита) |
...
### День 3–4 — Фаза 0, ещё 40 писем + IMAP helpers ✅ (Этап 3 код завершён)

```
[x] 20 писем / день — ⚠️ ручной тест не проводился
[x] Задача 19: IMAP helpers module (granite/email/imap_helpers.py) ✅
[x] Задача 5: follow-up при открытии + авто-отмена + счётчики ✅
[x] Задача 6: process_bounces.py (использует imap_helpers) ✅
[x] Задача 9: process_replies.py (использует imap_helpers) ✅
[x] Задача 11: process_followups.py ✅
[ ] Мониторинг bounce rate — ⚠️ нужен реальный запуск
[ ] Проверить что total_replied инкрементируется при ответе — ⚠️ нужен реальный запуск
[ ] Проверить A/B stats endpoint: /campaigns/{id}/ab-stats — ⚠️ нужен реальный запуск
```
...
[x] Задача 10: обновления фронтенда — ✅ CampaignWizard + CampaignDashboard + PostReplyButtons + TouchHistory
[x] Задача 12: ✅ Созданы docs/POST_REPLY_PLAYBOOK.md + docs/EMAIL_TEMPLATES.md
[x] Задача 13: ✅ cold_email_bitrix (id=4) добавлен в email_templates.json
```
...
|--------|------|-----------|
| v13 | 2026-04-28 | 4 баг-фикса (SEO-regex ×2, template name, yield_per/SQLite), возврат кода из v11, задача 19 (IMAP helpers), раздел «Принятые решения», задача 17 → Этап 2, миграция description, **Этап 3 завершён**: IMAP helpers + bounce/reply/follow-up + аудит (P3-H1..H3, P3-M1..M4, P3-L2) + реорганизация тестов (40 тестов в 4 feature-based файлах), **Этап 4 завершён**: CampaignWizard + CampaignDashboard + PostReplyButtons + TouchHistory + Reply API + 3 аудита (11+52+10 багов) + реорганизация тестов (28 тестов в 2 feature-based файлах) |
