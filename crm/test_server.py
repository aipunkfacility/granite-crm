...
def test_smtp_timeout():
    ...
    with patch("server.smtplib.SMTP") as mock_smtp:
        ...
        server.send_single_email("test@example.com", "<html>test</html>")
...
def test_logging_on_failure():
    ...
    with (
        ...
        server.send_single_email("bad@example.com", "<html>test</html>")

        assert mock_error.called, "logger.error should be called on failure"
        calls = [str(c) for c in mock_error.call_args_list]
        assert any("bad@example.com" in c for c in calls), (
            f"Error log should contain email address. Calls: {calls}"
        )
